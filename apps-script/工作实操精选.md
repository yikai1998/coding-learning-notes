...
```gs
  ...
  // 主循环按 region/entity 分配采样任务
  Object.keys(entityRows).forEach(region => {
    Logger.log(`处理region: ${region}, 总数: ${regionRows[region].length}, 抽样: ${determineRegionSampleSize(regionRows[region].length)}`);
    Object.keys(entityRows[region]).forEach(entity => {
      const groupRows = entityRows[region][entity];
      const rowsCount = groupRows.length;
      // 拿 region 总目标，按 entity 下占比分配
      const sampleTarget = Math.ceil(determineRegionSampleSize(regionRows[region].length) * (rowsCount / regionRows[region].length));
      Logger.log(`处理region: ${region}, entity: ${entity}, 总数: ${rowsCount}, 抽样: ${sampleTarget}`);
      if (sampleTarget == 0) {return 0;}  // entity分不到名额，跳过

      // stp拆组（确保兼容布尔和字符串, 用!!）
      const stpTrueRows  = groupRows.filter(r => !!r[pos_stp] && String(r[pos_stp]).toLowerCase() === 'true');
      const stpFalseRows = groupRows.filter(r => !r[pos_stp] || String(r[pos_stp]).toLowerCase() === 'false');
      const stpTrueCount = stpTrueRows.length;
      const stpFalseCount = stpFalseRows.length;

      // stp true/false采样名额计算
      let sampleTarget_stpTrue = Math.min(
        Math.floor(sampleTarget * (stpTrueCount / rowsCount)),  // 按比例分配
        30,  // stp=true最多取30
        stpTrueCount
      );
      let sampleTarget_stpFalse = sampleTarget - sampleTarget_stpTrue;
      if (stpFalseCount < sampleTarget_stpFalse) {
        // false部分不够，补给true，但不得超额
        sampleTarget_stpTrue += (sampleTarget_stpFalse - stpFalseCount);
        sampleTarget_stpFalse = stpFalseCount;
        sampleTarget_stpTrue = Math.min(sampleTarget_stpTrue, 30, stpTrueCount);
      }

      // stp=true的分层采样
      const stpTrueSamples = stratifyByCRR(stpTrueRows, sampleTarget_stpTrue, pos_crr);

      // stp=false的优先抽sca_team true
      const scaTeamTrueRows  = stpFalseRows.filter(r => !!r[pos_sca] && String(r[pos_sca]).toLowerCase() === 'true');
      const scaTeamFalseRows = stpFalseRows.filter(r => !(!!r[pos_sca] && String(r[pos_sca]).toLowerCase() === 'true'));
      let sampleTarget_scaTeamTrue = Math.min(10, sampleTarget_stpFalse, scaTeamTrueRows.length);
      ...
      // 合并结果
      finalSamples = finalSamples.concat(stpTrueSamples, scaTeamTrueSamples, scaTeamFalseSamples);
    });
  });
```
