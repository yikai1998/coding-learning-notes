...
(?:pattern) 是一个普通的非捕获分组，它直接参与整体的匹配过程，不保存匹配结果。
(?=pattern) 是一个正向肯定预查，它首先检查后面的 pattern 是否匹配，如果匹配，再检查前面的内容是否匹配，整个表达式只有在前后都匹配的情况下才成功。
...
假设我们有以下字符串：
Hello, World! 123
使用 \S 匹配：会匹配 "Hello,"、"World!" 和 "123" 中的所有字符，但不包括空格。
使用 \w 匹配：会匹配 "Hello"、"World" 和 "123" 中的所有字母数字字符，不包括逗号和空格。
...
解释：
(\b\w+\b)：这是一个捕获组，它匹配一个完整的单词（\w+ 匹配一个或多个字母数字字符或下划线，\b 是单词边界）。
\s+：匹配一个或多个空白字符。
\1：这是一个反向引用，它引用第一个捕获组匹配的完整单词。
